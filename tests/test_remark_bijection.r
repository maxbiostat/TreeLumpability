...
if(compute){
 ...
 save(SPR.mat,
            file = paste0("SPR_matrix_n=", n, ".RData"))
}else{
 load(paste0("SPR_matrix_n=", n, ".RData"))
}
...
n.sizes.decomp <- unlist(lapply(all.trees,
                                function(t) neighbourhood_size(t)$n_size))
...
A1.c.pos <- lapply(in.neighbourhood.pos,
                     function(x) intersect(x, S1.c.pos))
...
Ntauxcs <- unlist(lapply(parts,
                         function(x) neighbourhood_size(x$tau_x_c)$n_size))
