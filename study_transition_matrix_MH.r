...
get_stuff <- function(clade) {
    ...
    all.errors <- do.call(rbind,
                          apply(grid.pos, 1,
                                function(row) {
                                  ...
                                  y.pos <- row[2]
                                  sum_x <-
                                    sum(trans.mat.MH[x.pos, S0.c.pos])
                                  sum_y <-
                                    sum(trans.mat.MH[y.pos, S0.c.pos])
                                  return(
                                    data.frame(
                                      index.x = x.pos,
                                      index.y = y.pos,
                         ...
                         apply(grid.pos, 1,
                               function(row) {
                                 ...
                                 return(data.frame(
                                   index.x = x.pos,
                                   index.y = y.pos,
                                   rspr_dist = rspr(all.trees[[x.pos]],
                                                    all.trees[[y.pos]])
                                 ))
                               }))
    all.dists$ntaxa <- n
  ...
  Nphixcs <- unlist(lapply(parts,
                           function(x)
                             neighbourhood_size(x$phi_x_c)$n_size))
  
  A0s.sizes <- rep(NA, K)
  A0s.sizes[hasclade] <-
    neighbourhood.sizes[hasclade] - Nxprimes + Nphixcs
  
  all.errors <- compute_all_errors(clade)
  dists <- compute_all_dists(clade)
  
  all.errors.dt <- tibble::tibble(merge(
    all.errors,
    dists,
...
nontrivial <-
  intersect(which(clade.sizes > 1), which(clade.sizes < n))
all.ntclades <- all.splits[nontrivial]
all.errors <- parallel::mclapply(all.ntclades,
                                 function(clade)
                                   get_stuff(clade), mc.cores = 8)
